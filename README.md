...
### Live demo — drag to spin the wheel

The **Gátople** is a two-disc spinner: the outer disc is the function/role
layer (glyph + color fixed at each clock position), the inner disc carries the
12 notes and rotates freely. Drag, click any note to set it as tonic, or use
the ◀ ▶ arrows. The piano keyboard and the guitar fretboard repaint together.

* **Static (zero-build):** `docs/gatople/index.html` — opens in any browser.
  Variant: `docs/gatople/flow/` (auto-spin + scale select + flow animations).
* **React + TypeScript:** `web/` — Vite project, `npm run dev` to develop,
  `npm run build` to ship a `dist/` bundle (~64 KB gzipped).

Both consume the same `docs/gatople/data.json`, which is regenerated by
`uv run python scripts/build_gatople_data.py`. A drift test
(`tests/integration/test_gatople_data_sync.py`) fails loudly if the JSON
falls behind the Python data model.

Two color systems coexist — both come from canonical sources, neither is a
generic palette:

* **`WHEEL_HEX`** — the chromatic gradient inside the cat-face mandala
  (yellow at 12, orange at 1–2, red at 4, violet at 6, teal at 9, lime at 11),
  matching the Logo Gátople artwork.
* **`CARTA_HEX`** — the cover-legend four-color rule (red / green / blue /
  ivory) used by the piano and fretboard sticker overlays.
